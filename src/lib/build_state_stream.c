...
static b32 build_state_append_remove_unlocked(String path, Build_State *state, Bob_Path output)
{
...
b32 build_state_append_remove(String path, Build_State *state, Bob_Path output)
{
	if (!state || !state->initialized) return false;
	platform_lock_mutex(&state->mutex);
	b32 result = build_state_append_remove_unlocked(path, state, output);
	platform_unlock_mutex(&state->mutex);
	return result;
}
