                     ...
                     i32 verbosity, b32 verbosity_override)
{
...

    {
        ...
        if (!platform_file_info(path, &build_file)) {
            ...
            if (platform_current_directory(scratch.arena, &working_directory)) {
                log_error("%s: file not found (working directory: %s)",
    ...
    for (i = 0; i < task_count; ++i) {
        ...
        if (error != GRAPH_OK) {
            log_error("%s: unable to create task '%s': %s", path,
                      loaded.tasks[i].name, graph_error_str(error));
            goto cleanup;
        }
    }
    ...
    for (i = 0; i < task_count; ++i) {
        ...
        for (dependency_index = 0;
             dependency_index < loaded.tasks[i].dependency_count;
             ++dependency_index) {
            u32 dependency = loaded.tasks[i].dependencies[dependency_index];
            Graph_Error error;

            if (dependency >= task_count) {
                log_error("%s: task '%s' has invalid dependency %u", path,
                          loaded.tasks[i].name, dependency);
                goto cleanup;
            }
            {
                Profile_Scope scope = profile_scope_begin("construct graph");
                error = graph_add_dependency(graph, (Node_Id)i,
                                             (Node_Id)dependency);
                profile_scope_end(&scope);
            }
            if (error != GRAPH_OK) {
                log_error("%s: unable to add dependency to '%s': %s", path,
                          loaded.tasks[i].name, graph_error_str(error));
                goto cleanup;
            }
        }
...
int main(int argument_count, char **arguments)
{
    ...
    for (argument_index = 1; argument_index < argument_count; ++argument_index) {
        if (strcmp(arguments[argument_index], "--verbose") == 0) {
            ...
            if (argument_index + 1 < argument_count) {
                ...
                if (*end == 0) {
                    if (parsed > INT32_MAX) {
                        log_error("invalid verbosity level: %s",
        ...
        } else if (strcmp(arguments[argument_index], "--workers") == 0 &&
    ...
    }
    {
        ...
        result = run_build(build_path, worker_count, worker_override,
