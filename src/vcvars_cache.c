...
static b32 string_equal_insensitive(String left, String right)
{
    u64 i;
    if (left.size != right.size) return false;
    for (i = 0; i < left.size; ++i) {
        if (ascii_lower((u8)left.data[i]) != ascii_lower((u8)right.data[i])) {
            return false;
        }
    }
    return true;
...
static b32 string_starts_with(String text, String prefix)
{
    return prefix.size <= text.size &&
        memcmp(text.data, prefix.data, (size_t)prefix.size) == 0;
}

static b32 string_ends_with(String text, String suffix)
{
...
static b32 string_is(String text, const char *literal)
{
    return string_equal(text, string_from_cstring(literal));
}

static u32 count_lines(String text)
{
    u64 i;
    u32 count = text.size ? 1 : 0;
...
static b32 parse_capture(Arena *arena, String capture,
                         Env_Table *before, Env_Table *after)
{
    u32 capacity = count_lines(capture);
    u64 start = 0;
    b32 found_marker = false;

    before->items = arena_push_zero_aligned(arena,
        sizeof(*before->items) * capacity, _Alignof(Env_Entry));
    after->items = arena_push_zero_aligned(arena,
        sizeof(*after->items) * capacity, _Alignof(Env_Entry));
    ...
    while (start < capture.size)
    {
        ...
        String line;
        Env_Entry entry;
        Env_Table *destination;
        while (end < capture.size && capture.data[end] != '\n') ++end;
        line = string_slice(capture, start, end - start);
        if (line.size && line.data[line.size - 1] == '\r') --line.size;
        ...
        destination = found_marker ? after : before;
...
static b32 push_cache_rule(Arena *arena, const char *action,
...
static b32 build_cache(Arena *arena, Env_Table *before,
                       Env_Table *after, String *result)
{
    u64 start = arena_mark(arena);
    u32 i;
    if (!arena_push_text(arena, VCVARS_CACHE_HEADER) ||
        !arena_push_char(arena, '\n')) return false;
    ...
    for (i = 0; i < after->count; ++i)
    {
        ...
        if (is_path_list_variable(new_entry->name)) {
            ...
            delta = build_path_list_delta(scratch.arena, old_entry->value,
                                          new_entry->value);
            ...
            delta = string_slice(new_entry->value, 0,
                                 new_entry->value.size - old_entry->value.size);
            if (delta.size && !push_cache_rule(arena, "prepend", new_entry->name, delta)) {
                return false;
            }
        } else if (string_starts_with(new_entry->value, old_entry->value)) {
            delta = string_slice(new_entry->value, old_entry->value.size,
                                 new_entry->value.size - old_entry->value.size);
...
static b32 cache_paths(char *directory, u32 directory_size,
                       char *path, u32 path_size)
{
    Scratch scratch = get_scratch();
...
b32 vcvars_cache_apply(String cache)
{
    ...
    while (start < cache.size)
    {
        ...
        for (separator = space + 1;
             separator < line.size && line.data[separator] != '=';
             ++separator) {}
        if (space == 0 || separator >= line.size) return false;
        if (!apply_rule(string_slice(line, 0, space),
...
b32 vcvars_cache_refresh(char *path, u32 path_size)
{
    static const char command[] =
        "cmd.exe /d /s /c \"set&echo " VCVARS_CAPTURE_MARKER
        "&call vcvars64 >nul&&set\"";
    ...
    if (!arena.data || !cache_paths(directory, sizeof(directory), path, path_size)) goto cleanup;
    ...
    if (!parse_capture(&arena, capture, &before, &after) ||
        !build_cache(&arena, &before, &after, &cache)) {
