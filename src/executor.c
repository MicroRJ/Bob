...
static b32 task_needs_rebuild(const Graph *graph, Node_Id node,
                              const Task *task, const b32 *rebuilt)
{
    ...
    }
    {
        ...
        scan_ok = c_include_scan(task->inputs, task->input_count,
                                 task->include_directories,
                                 task->include_directory_count,
                                 task->command_line, &scan);
        profile_scope_end(&scope);
        if (!scan_ok ||
            scan.unresolved_quoted_include) {
            return true;
        }
    ...
    while (!graph_is_finished(graph)) {
        for (i = 0; i < worker_count; ++i) {
            ...
            while (!worker->busy && graph_take_ready(graph, &node)) {
                ...
                if (!needs_rebuild) {
                    if (verbosity >= 0) {
                        logger_log(LOG_LEVEL_INFO, "up-to-date", "%s",
                                   graph_node_name(graph, node));
                        if (verbosity >= 1) {
                            printf("  command: %s\n", task->command_line);
                        }
                    }
...

        {
            ...
            if (wait_result < WAIT_OBJECT_0 ||
                wait_result >= WAIT_OBJECT_0 + worker_count) {
                internal_error = true;
                break;
            }
            ...
            if (verbosity >= 0) {
                logger_log(succeeded ? LOG_LEVEL_SUCCESS : LOG_LEVEL_ERROR,
                           succeeded ? "succeeded" : "failed", "%s",
                           graph_node_name(graph, worker->node));
