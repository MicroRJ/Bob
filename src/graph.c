...
static b32 reserve(Graph *graph, void **memory, u32 element_size, u32 count,
                   u32 *capacity, u32 needed, u64 alignment)
{
    size_t new_capacity;
    void *new_memory;
...
Graph_Error graph_add_dependency(Graph *graph, Node_Id node_id,
...
Graph_Error graph_prepare(Graph *graph)
{
    ...
    if (graph->node_count > 0) {
        graph->ready = graph_push(graph,
                                  graph->node_count * sizeof(*graph->ready),
                                  _Alignof(Node_Id));
        if (!graph->ready) {
            return GRAPH_ERROR_OUT_OF_MEMORY;
        }
...
Node_Id graph_node_dependency(const Graph *graph, Node_Id node, u32 index)
{
    if (!valid_node(graph, node) ||
        index >= graph->nodes[node].dependencies.count) return GRAPH_INVALID_TASK;
