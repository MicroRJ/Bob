...
char *error, size_t error_size)
{
   ...
   if (value.type != ELF_VALUE_TYPE_TABLE) {
      snprintf(error, error_size, "%s for '%s' must be a table", field_name, task_name);
      return 0;
   }

   table = value.as.table;
   count = elf_table_length(table);
   items = arena_push_zero_aligned(arena, count * sizeof(*items), _Alignof(char *));
   if (count && !items) {
   ...
   *count_out = count;
   ...
   for (i = 0; i < count; ++i)
   {
      ...
      if (!items[i]) {
         snprintf(error, error_size, "%s for '%s' must contain strings", field_name, task_name);
         return 0;
      }
   }
   return 1;
...
int elf_load_task_list(const char *path, Arena *arena, Task_Array_Desc *result)
{
   ...
   task_tables = arena_push_zero_aligned(scratch.arena, result->count * sizeof(*task_tables), _Alignof(elf_Table *));
   ...
   {
      Task_Desc *output = &result->tasks[i];
      elf_ValueView dependencies_value = field(state, task_tables[i], "dependencies");
      uint32_t dependency;

      if (dependencies_value.type == ELF_VALUE_TYPE_NIL) { continue; }
      if (dependencies_value.type != ELF_VALUE_TYPE_TABLE) {
         snprintf(result->error, sizeof(result->error), "dependencies for '%s' must be a table", output->name);
         goto cleanup;
      }
